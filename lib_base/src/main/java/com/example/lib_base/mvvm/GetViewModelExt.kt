...
import com.example.lib_base.mvvm.base.BaseViewModel
 ...
 */
@Deprecated("已过时的方法，现在可以直接使用Ktx函数 viewmodels()获取", ReplaceWith(
    "ViewModelProvider(this, ViewModelProvider.AndroidViewModelFactory(application)).get(VM::class.java)",
 ...
 */
@Deprecated("已过时的方法，现在可以直接使用Ktx函数 viewmodels()获取", ReplaceWith(
    "ViewModelProvider(this, ViewModelProvider.AndroidViewModelFactory(this.requireActivity().application)).get(VM::class.java)",
    "androidx.lifecycle.ViewModelProvider",
    "androidx.lifecycle.ViewModelProvider"
)
)
 ...
 * 提示，在fragment中调用该方法时，请在该Fragment onCreate以后调用或者请用by lazy方式懒加载初始化调用，不然会提示requireActivity没有导致错误
 */
@Deprecated("已过时的方法，现在可以直接使用Ktx函数 activityViewModels()获取", ReplaceWith(
    "ViewModelProvider(requireActivity(), ViewModelProvider.AndroidViewModelFactory(this.requireActivity().application)).get(VM::class.java)",
    "androidx.lifecycle.ViewModelProvider",
    "androidx.lifecycle.ViewModelProvider"
)
)
